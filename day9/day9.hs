...
sol1 :: IO ()
sol1 = do
    prog <- parseIntcode <$> readFile "inp9.txt"
    print $ show $ runIntcode prog [1]

sol2 :: IO ()
sol2 = do
    prog <- parseIntcode <$> readFile "inp9.txt"
    print $ show $ runIntcode prog [2]
...
getOutput :: Writer [Integer] a -> Maybe Integer
getOutput = listToMaybe . snd . runWriter

test :: IO ProgState
test = do
    prog <- parseIntcode <$> readFile "inp9.txt"
    return $ ProgState [1] prog 0 0
